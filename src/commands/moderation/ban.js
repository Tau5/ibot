exports.execute = async (client, ctx) => {
  ...
  if (search) {
    ...
    ctx.channel.send(client.I18n.translate`❌ You must mention or specify a user to ban!`);
  }
  ...
  if (!member.bannable || ctx.member.highestRole.comparePositionTo(ctx.guild.me) <= 0) return ctx.channel.send(client.I18n.translate`❌ The specified member (**${member.user.tag}**) cannot be banned!`);
